...
uses `gpt-6-luna`, medium reasoning and trace + knowledge + grounding (D).
...
- Release: `research-explanations-v2`. No model picker or strategy picker.
...
## Model update — 25 September 2026

Release `research-explanations-v2` switches the product model to `gpt-6-luna`.
Reasoning effort and prompt strategy remain unchanged. The acceptance record
below applies to v1 with `gpt-5.6-luna`, not to the new model.
