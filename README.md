...
Compiler optimisation and program reduction visualiser built with Next.js, tRPC and Prisma. Local development uses PostgreSQL and a Docker-based LLVM service. Optional AI explanations use `gpt-6-luna` with trace + knowledge + grounding.
...
Use a key with access to `gpt-6-luna`. The key stays on the server; do not commit it. Without a key the visualisers still work, but explanation requests report a configuration error. API calls may incur charges. See [AI_EXPLANATIONS.md](AI_EXPLANATIONS.md) for the feature and research exports.
