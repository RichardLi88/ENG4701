...
- `npm run test:e2e:llvm` - run real local C/C++ LLVM success and failure cases (requires `./start-llvm.sh`)
...
- Start the LLVM service before `npm run test:e2e:llvm`. Set `LLVM_SERVICE_URL` to test a non-default local service URL.
